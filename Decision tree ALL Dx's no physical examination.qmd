---
title: "Decision tree ALL Dx's no physical examination"
format: html
---
...
# Define the 20 assessment values
assessment_values <- c("ACL", "ACL MEN", "PFI", "MODMCL", "MMT", "MILMCL", 
                       "ACL MMT", "LMT", "OA", "MMT MILMCL", "OTHER", "ACL LMT", 
                       "PFP", "ACL MODMCL", "ACL MEN MILMCL", "BC", "MMT MODMCL", 
                       "ACL MILMCL", "MEN", "ACL MODMCL MEN")

# Filter filtered_df for the specific assessment values
filtered_df_work <- filtered_df %>%
...
filtered_df_work <- filtered_df_work %>% select(-all_of(columns_to_remove))
...
write.csv(accuracy_measures, file = "knee_injury_diagnosis_accuracy_with_LR_20_diagnoses.csv", row.names = FALSE)

# Print the total cases again for confirmation
print(paste("Total cases with specified assessment values:", total_cases))
